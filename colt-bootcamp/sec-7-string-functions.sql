...
SELECT SUBSTRING('Hello World', 1, 4); -- we get "Hell"
SELECT SUBSTRING('Hello World', 2, 4); -- we get "ello"

-- We can also just provide a single number
-- This tells MySQL "Just go to the end"
SELECT SUBSTRING('Hello World', 7); -- we get "World"

-- We can use a negative number to count backwards from the end of the string
SELECT SUBSTRING('Hello World', -1); # gets the last character in the string
...
SELECT 
	CONCAT(
        ...
        '-',
        LOWER(SUBSTRING(title, 1, 1))) AS 'Book Code'
FROM books;
